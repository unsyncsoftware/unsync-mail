...
async function main() {
    ...
    try {
        ...
        const portalResult = await (0, sync_1.sendEmail)({
            smtp: { host: "smtp.example.test", port: 587, secure: false },
            draft: {
                from: "local@example.test",
        ...
        assert.match(missingKeyTransport.sent[0]?.text?.toString() ?? "", /This message was sent securely using Unsync Mail/);
        assert.doesNotMatch(missingKeyTransport.sent[0]?.text?.toString() ?? "", /Do not send this as plaintext/);
